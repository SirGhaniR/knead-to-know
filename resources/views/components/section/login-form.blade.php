<div
  ...
  <p class="mb-18 text-center">Welcome! Login here to prove that you're an admin.</p>
  <x-ui.divider class="absolute left-0 right-0 top-36 mx-auto" />
  <form action="{{ url('/login') }}" method="post">
    @csrf
    <div class="mb-4 flex flex-col gap-3">
      <x-ui.input :name="'email'" :id="'email'" :placeholder="'Email'"
        class="px-8! py-4! outline-gray-500! rounded-0!" />
      <x-ui.input :name="'password'" :id="'password'" :placeholder="'Password'"
        class="px-8! py-4! outline-gray-500! rounded-0!" />
    </div>
    <button type="submit"
