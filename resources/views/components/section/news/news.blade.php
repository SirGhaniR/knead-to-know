...
<div
  ...
  @if ($featuredNews)
    ...
    <div class="flex flex-1 flex-col justify-center gap-4 sm:gap-6 lg:gap-10" data-aos="fade-left" data-aos-delay="150"
      ...
      <p class="text-2xl font-bold uppercase transition duration-150 hover:scale-105 sm:text-3xl" data-aos="fade-down"
        data-aos-delay="100" data-aos-duration="300">{{ $featuredNews->title }}</p>
      <p class="text-sm" data-aos="fade-up" data-aos-delay="150" data-aos-duration="400">
        {!! nl2br(e(Str::limit($featuredNews->content, 500))) !!}
      </p>
      <x-ui.button :href="route('news.show', $featuredNews->id)" class="w-full transition duration-150 hover:scale-105 sm:w-auto md:w-fit lg:w-fit"
        data-aos="fade-up" data-aos-delay="200" data-aos-duration="400">BACA SELENGKAPNYA</x-ui.button>
    </div>
  @else
...
<div class="px-4 py-12 sm:px-6 md:px-8 lg:px-20 lg:py-14" data-aos="fade-up" data-aos-offset="50"
  data-aos-duration="400">
  <p class="mb-8 mt-1 text-center text-lg font-bold transition duration-150 hover:scale-105 sm:mb-10 sm:text-xl md:mb-12 lg:text-left"
    data-aos="fade-down" data-aos-delay="80" data-aos-duration="300">BERITA LAINNYA</p>
  @if ($news)
